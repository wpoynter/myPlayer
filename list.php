...
$getID3 = new getID3;
$counter =0;
//$DirectoriesToScan[] = '/Users/williampoynter/Music'; // change to whatever directory you want to scan
					...
					while (($file = readdir($dir)) !== false) {
						if (($file != '.') && ($file != '..')) {
							...
							if (is_dir($RealPathName)) {
								...
								echo '<td><a href="?path=' . $_GET['path'] . '&song=' . str_replace(")", ")",str_replace("(", "(",str_replace(" ", " ",$ThisFileInfo['filenamepath']))) . '">' . substr((!empty($ThisFileInfo['comments_html']['title'])  ? implode('<BR>', $ThisFileInfo['comments_html']['title'])  : '&nbsp;'),0,40).'</a></td>';
